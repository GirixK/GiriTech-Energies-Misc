...
public class CastingRecipes {
    ...
    public static void init(Consumer<FinishedRecipe> provider) {
        ...
        for (var entry: moltenMetals.entrySet()) {
            ...
            GTRecipeBuilder screwdriverBuilder =
                    TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_screwdriver_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
                    .notConsumable(GTEMiscItems.SINGLE_USE_SCREWDRIVER_CAST)
                    .duration(entry.getValue().getRight()*20)
                    .EUt(96);

            GTRecipeBuilder sawBuilder =
            TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_saw_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
                    .notConsumable(GTEMiscItems.SINGLE_USE_SAW_CAST)
                    .duration(entry.getValue().getRight()*20)
                    .EUt(96);

            GTRecipeBuilder fileBuilder =
            TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_file_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
                    .notConsumable(GTEMiscItems.SINGLE_USE_FILE_CAST)
                    .duration(entry.getValue().getRight()*20)
                    .EUt(96);

            GTRecipeBuilder wrenchBuilder =
            TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_wrench_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
            ...
            GTRecipeBuilder crowbarBuilder =
            TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_crowbar_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
        ...
        for (var entry : moltenRubbers.entrySet()) {
            ...
            GTRecipeBuilder malletBuilder =
            TOOL_CASTING_RECIPES.recipeBuilder("cast_single_use_mallet_from_" + entry.getKey().getTranslationKey())
                    .inputFluids(entry.getKey().copy())
